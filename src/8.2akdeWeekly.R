...
tempPoints <- readRDS("./src_outputs/finalTemp.rds") %>%
  dplyr::mutate(
    week = lubridate::floor_date(dateTimeAEST, unit = "week"),
    weekID = paste0(BandNumber, "_", as.character(week)),
    x = lon,
    y = lat
  ) %>%
  dplyr::select(BandNumber, LOCBANDED, week, weekID, dateTimeUTC, dateTimeAEST, x, y, lon, lat) %>%
...
# Save all fits
save(allFits, file = './src_outputs/ctmmFitsweekly_models.Rdata')
save(tel, file = './src_outputs/ctmmTelemetryweekly.Rdata')

load('./src_outputs/ctmmFitsweekly_models.Rdata')
load('./src_outputs/ctmmTelemetryweekly.Rdata')
...
save(allAKDE, file = './src_outputs/allAKDE_weekly.Rdata')
load(file = './src_outputs/allAKDE_weekly.Rdata')

rownames(summary(allAKDE[[101]])$CI)
...
akdeDFweekly <- cbind(weekIDs, areaDF) %>%
  data.frame() %>%
...
saveRDS(akdeDFweekly, "./src_outputs/akde_weeklyFinal.rds")


# Overlap measurements ----------------------------------------------------

load(file = './src_outputs/allAKDE_weekly.Rdata')  # allAKDE
names(allAKDE) <- names(tel)  # 'tel' has weekIDs
akde_by_individual <- split(allAKDE, sapply(strsplit(names(allAKDE), "_"), `[`, 1))

names(allFits)


chk<-overlap(object=allAKDE ,method="Bhattacharyya",level=0.95) 



UDS <- akde(tel[2:5],allFits[2:5])

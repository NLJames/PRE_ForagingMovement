...
bandSex<-readRDS("./src_outputs/finalTemp.rds")%>%
  distinct(BandNumber, SEX, LOCBANDED)
...
dd<-tempPoints%>%
  ...
  dplyr::filter(featType=="offIsland")
...
# Get bare min
ddPrep<-dd%>%
  dplyr::select(dateTimeAEST, BandNumber, UTMx, UTMy)
...
ddist <- do.call('rbind', lsDist) %>%
  ...
  dplyr::filter(BandNumber!=BandNumber2)

unique(ddist$couple)
saveRDS(ddist, "./src_outputs/socialDist_offIsland.rds")
